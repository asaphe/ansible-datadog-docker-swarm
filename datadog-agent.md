...
> Docker labels can be added as tags - specify it using the config file or enviroment variables  
...
meaning we can potentially have the following structure:
```
...
basically the filename doesn't even have to be `conf.yaml`, meaning you can also have the following:
```
- /etc/datadog-agent/conf.d/ntp.d/
  |-prod.yaml
  |-dev.yaml
